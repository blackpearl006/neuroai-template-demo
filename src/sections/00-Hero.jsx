...
export default function Hero() {
  ...
  const hasBrain = !!hero.brain?.atlas;
  ...
  const brain = motionOK && !brainFailed ? (
    // Motion: borderless, slowly auto-rotating 3D mesh on a transparent canvas —
    // it composites straight onto the page, not a framed screenshot.
    <div className="relative hero-brain-mask h-[380px] lg:h-[460px]">
      <BrainBoundary onFail={() => setBrainFailed(true)}>
        <Suspense fallback={null}>
          ...
          >
            <HeroBrain onReady={() => setBrainReady(true)} />
          </div>
        </Suspense>
      </BrainBoundary>
      <a
    ...
    // Reduced-motion / no-WebGL: the instant static glass-brain, contained
    // (a black projection reads best framed).
    <figure className="rounded-2xl border border-rule/20 overflow-hidden bg-paper2">
  ...
  return (
    <header className="max-w-wide mx-auto px-6 pt-20 pb-14">
      <div className={hasBrain ? "grid lg:grid-cols-5 lg:gap-10 lg:items-center" : ""}>
        {/* Text column — ~60% on large screens */}
        <div className={hasBrain ? "lg:col-span-3" : ""}>
          {hero.eyebrow && (
          ...
          <p
            className="mt-5 font-serif text-xl text-ink2 max-w-[52ch] leading-relaxed"
        ...
        {/* Brain column — ~40% on large screens; stacks below the text otherwise */}
        {hasBrain && (
